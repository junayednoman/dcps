  ...
  const notifications = [
    {
      ...
      date: "5 mins ago",
      link: "https://youtu.be/2otoYNbFSBI?si=izlUGjIv5jp2CdbZ",
    },
    {
  ...
  return (
    <div className="dropdown relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <ul
          style={{
            display: notifications.length > 0 ? "block" : "flex",
            justifyContent: "center",
            alignItems: "center",
          }}
          className="dropdown-content absolute right-0 bg-white shadow-md mt-2 w-[300px] min-h-[300px] max-h-[330px] overflow-y-scroll z-10 p-6 space-y-4 text-sm capitalize border rounded-md notificationDropdown"
        >
          {notifications.length > 0 ? (
            notifications.map((item, i) => (
              <li key={i}>
                <a className="flex gap-2" href={item.link} target="_blank">
                  <div className="bg-primaryColor p-[5px] max-h-[25px] rounded-sm">
                    <FaBell className="text-sm text-white" />
                  </div>
                  <div className="space-y-2">
                    <h6 className="font-semibold">{item.title}</h6>
                    <span className="text-[13px] text-[#00000076] lowercase">
                      {item.date}
                    </span>
                  </div>
                </a>
